...
class ChannelGate(nn.Module):
    ...
    def __init__(self, in_channels, num_gates=None, return_gates=False,
                 gate_activation='sigmoid', reduction=15, layer_norm=False): # reduction 16
...
class Attentions(nn.Module):
    def __init__(self, filter_channels):
        ...
        self.conv1 = nn.Conv2d(filter_channels, filter_channels, 3, padding=1)
        self.attention_filter = 1

        self.attention_inv_filter = 1

    def attention(self, y):
        tanh_img = torch.nn.functional.tanh(y)
        probs_img = torch.nn.functional.softmax(tanh_img)
        return probs_img
    ...
    def forward(self, y):
        ...
        #print("type y", type(y))
        img = self.conv1(y)
        self.attention_filter = self.attention(img) * self.attention_inv_filter
        y = y * self.attention_filter
...
class Test(nn.Module):
    ...
    def __init__(self, num_classes, blocks, layers, channels, feature_dim=512, loss='softmax', IN=False, **kwargs):
        ...
        self.conv5 = Conv1x1(channels[3], channels[3])
        self.global_avgpool = nn.AdaptiveAvgPool2d(1)
        self.att1 = Attentions(channels[3])
        self.att2 = Attentions(channels[3])
        self.att3 = Attentions(channels[3])
        self.att4 = Attentions(channels[3])
        self.att5 = Attentions(channels[3])
        ...
        self.attentions = [self.att1, self.att2, self.att3, self.att4, self.att5]
        self.num_of_attentions = len(self.attentions)
        self._init_params()
    ...
    def forward(self, x, return_featuremaps=False):
        ...
        attention_list = []
        inv_attention = 1
        for attention_id in range(self.num_of_attentions):
            self.attentions[attention_id].attention_inv_filter = inv_attention
            if attention_id == 0:

                ix = self.attentions[attention_id](x)

                v = self.global_avgpool(ix)
                iv = v.view(v.size(0), -1)
                if self.fc is not None:
                    iv = self.fc(iv)

                iy = self.classifier(iv).unsqueeze(1)
                iv = iv.unsqueeze(1)

                attention_list.append(self.attentions[attention_id].attention_filter)

                cumulative_attention = attention_list[0]
                for prev_attention in attention_list[1:]:
                    tmp_cum_inv_attention = 1 - cumulative_attention
                    cumulative_attention = cumulative_attention + (prev_attention * tmp_cum_inv_attention)
                inv_attention = 1 - cumulative_attention
                continue
            ...
            iy = torch.cat((iy, y.unsqueeze(1)), 1)
            iv = torch.cat((iv, v.unsqueeze(1)), 1)
            #print(iy.shape, iv.shape)
            attention_list.append(self.attentions[attention_id].attention_filter)
        ...
        # if epoch < 15:
        #     print("pretrain classifiers: ", epoch)
        #     x = x.detach()
        #print("x", x)


        # if train:
        #     act_x = self.AttentionActivation(x).view(-1, x.shape[2] * x.shape[3])
        #     for img_idx, s_filters in enumerate(act_x):
        #         for filter_idx, filter in enumerate(s_filters):
        #             logs_Pans = self.map_classifier(filter.view(-1, x.shape[2] * x.shape[3]))
        #             my[img_idx, filter_idx, :] += logs_Pans.view(-1)
...
# Instantiation
##########
def test(num_classes=1000, pretrained=True, loss='softmax', **kwargs):
    # standard size (width x1.0)
    model = Test(num_classes, blocks=[OSBlock, OSBlock, OSBlock], layers=[2, 2, 2],
                  channels=[64, 256, 384, 512], loss=loss, **kwargs, feature_dim=512)
    if pretrained:
